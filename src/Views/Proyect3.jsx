...
const getImages = (id) => {
  ...
  const dataTorotoServices = () => data.map(item => item.services)[2];

    return ( 
      <section className="section2">
        <Header/>
        <DescriptionProyect
            location= {data.map(item => item.location)[2]}
            nameProyect = {data.map(item => item.name)[2]}
            description ={data.map(item => item.description)[2]}
            descriptionProblem = {data.map(item => item.problem)}
            imgProyect = {getImages(data.map(item => item.id)[2])}
