...
const getImages = (id) => {
  ...
  const dataTorotoServices = () => data.map(item => item.services)[1];
    ...
    return ( 
      <section className="section2">
        <Header/>
        <DescriptionProyect
            location= {data.map(item => item.location)[1]}
            nameProyect = {data.map(item => item.name)[1]}
            description ={data.map(item => item.description)[1]}
            descriptionProblem = {data.map(item => item.problem)}
            imgProyect = {getImages(data.map(item => item.id)[1])}
        ...
        <Impact
          cardsImpact= {
            data.map(item => 
               <div key= {item.id}>
                  <CardsImpact
                    value={item.impact[1].value}
                    name ={item.impact[1].name}
                  />
               </div>
            )}/>
        <About/>
